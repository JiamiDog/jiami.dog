---
title: "【AWS】CloudFront免费CDN详细配置入门"
slug: "aws-cloudfront-free-cdn-detailed-configuration-getting"
source_id: "3169"
canonical_url: "https://jiami.dog/3169.html"
date_local: "2021-12-29T17:26:11"
date_published: "2021-12-29T09:26:11Z"
date_modified_local: "2021-12-29T17:36:16"
date_modified: "2021-12-29T09:36:16Z"
draft: false
featured_image_url: "https://jiami.dog/wp-content/uploads/2021/12/2021-12-27_09-14-09.jpg"
featured_image_alt: "AWS CloudFront入门配置教程的宣传图，包含AWS和CloudFront标志、卡通角色以及中文标题。"
categories:
  - "资源攻略"
tags:
  - "aws"
...
> 本文同步自 [jiami.dog 官方原文](https://jiami.dog/3169.html)，以官网版本为准。

![AWS CloudFront入门配置教程的宣传图，包含AWS和CloudFront标志、卡通角色以及中文标题。](https://jiami.dog/wp-content/uploads/2021/12/2021-12-27_09-14-09.jpg)

![](https://i0.wp.com/jiami.dog/wp-content/uploads/2021/12/2021-12-27_09-14-09-1.jpg?resize=1200%2C600&ssl=1)

在2021年的11月，亚马逊AWS对他的Always Free策略做出了一些改变，这篇文章主要介绍如何利用其中的CloudFront资源，一起来看看吧~
...
![](https://i0.wp.com/jiami.dog/wp-content/uploads/2021/12/2021-12-28_04-24-19-1.jpg?resize=777%2C259&ssl=1)

首先最开始的是源站配置，第一项`Origin domain`，只能填入一个解析到你源站的[域名](https://jiami.dog/tag/domain-names "域名")。如图如果你不想自己解析的话，可以利用`nip.io`项目在IP末尾添加`.nip.io`解析到源站IP。`Protcol`是取源协议，根据你的具体需求选择HTTP/HTTPS回源亦或者是匹配访问。

![](https://i0.wp.com/jiami.dog/wp-content/uploads/2021/12/2021-12-28_04-34-04-1.jpg?resize=926%2C357&ssl=1)

接下来是去设置默认的[缓存](https://jiami.dog/tag/cache "缓存")行为，其中第一条是访问设置`Viewer`。`Viewer protocol policy`根据你对外访问需要决定是否将HTTP跳转到HTTPS；`Allowed HTTP methods`为允许的请求头，可以根据实际需要选择，简单概括文件下载场景可选第一条、静态网站可选第二条、动态网站必须选择第三条，如果无法明确判定自己需要直接选择第三条全部支持即可。

![](https://i0.wp.com/jiami.dog/wp-content/uploads/2021/12/2021-12-28_04-42-59-1.jpg?resize=929%2C525&ssl=1)

第二条是缓存设置和源请求设置`Cache key and origin requests`，作为默认规则建议分别选择`CachingDisabled`禁止缓存以及`AllViewer`将所有访客请求头转发到源站。

![](https://i0.wp.com/jiami.dog/wp-content/uploads/2021/12/2021-12-28_04-47-59-1.jpg?resize=1067%2C325&ssl=1)

其他未提到的项目暂时保持默认（各个自定义名称的条目除外，看你要不要自己命名），然后拉到最下面点击`Create distribution`创建CDN资源。

![](https://i0.wp.com/jiami.dog/wp-content/uploads/2021/12/2021-12-28_04-51-44-1.jpg?resize=1197%2C436&ssl=1)
...
![](https://i0.wp.com/jiami.dog/wp-content/uploads/2021/12/2021-12-28_05-01-44-1.jpg?resize=784%2C225&ssl=1)

接下来就是比较重要的绑定自己的域名，在`Alternate domain name`可以添加一个或多个自己的域名到这个资源，随后可以前往DNS将自定义域名通过CNAME指向前文提到的CloudFront默认域名。

![](https://i0.wp.com/jiami.dog/wp-content/uploads/2021/12/2021-12-28_05-12-49-1.jpg?resize=1763%2C380&ssl=1)
...
![](https://i0.wp.com/jiami.dog/wp-content/uploads/2021/12/2021-12-28_05-18-34-1.jpg?resize=1367%2C644&ssl=1)

这里推荐签发Amazon的证书，有效期395天且支持泛域名。如图填入自己的域名，选择合适的验证方式添加对应的记录即可，验证完成后很快就能够签发成功。

![](https://i0.wp.com/jiami.dog/wp-content/uploads/2021/12/2021-12-28_05-21-50-1.jpg?resize=950%2C557&ssl=1)

准备好证书后回到我们的CDN设置页面，在`Custom SSL certificate`选项卡下选择你刚刚签发的证书，安全策略建议选择`TLSv1.1_2016`以获得较为广泛的兼容性。

![](https://i0.wp.com/jiami.dog/wp-content/uploads/2021/12/2021-12-28_10-51-25-1.jpg?resize=1076%2C307&ssl=1)
...
![](https://i0.wp.com/jiami.dog/wp-content/uploads/2021/12/2021-12-28_11-03-48-1.jpg?resize=1789%2C544&ssl=1)
...
![](https://i0.wp.com/jiami.dog/wp-content/uploads/2021/12/2021-12-28_11-20-24-1.jpg?resize=1246%2C713&ssl=1)
...
![](https://i0.wp.com/jiami.dog/wp-content/uploads/2021/12/2021-12-28_11-31-17-1.jpg?resize=1817%2C539&ssl=1)

配置Path pattern是本节内容的核心，在一开始创建的引导中我们已将默认路径`Default (*)`设置为不缓存任何内容，因此针对每一种我们需要缓存的内容都要单独配置缓存规则。

![](https://i0.wp.com/jiami.dog/wp-content/uploads/2021/12/2021-12-28_11-47-27-1.jpg?resize=798%2C185&ssl=1)
...
| /`*`.jpg | 指定CDN资源内所有`jpg`后缀文件 |
| /`*`.css`*` | 指定CDN资源内所有`css`后缀及包含参数的访问 |
| /files/`*`.gif | 指定`/files/`路径下所有`gif`后缀文件 |
| /a`??`.mp3 | 指定CDN资源内以a开头的`mp3`后缀文件 |
...
![](https://i0.wp.com/jiami.dog/wp-content/uploads/2021/12/2021-12-28_12-39-46-1.jpg?resize=1289%2C618&ssl=1)

使用CloudFront需要注意自己是否超出免费额度，超出后的价格是很贵的。账单是每天出一次，没有实时统计提供（控制面板的流量统计也有几个小时延迟），因此请不要将其用于易受攻击的站点。

![](https://i0.wp.com/jiami.dog/wp-content/uploads/2021/12/2021-12-28_12-18-16-1.jpg?resize=1567%2C498&ssl=1)
...
![](https://i0.wp.com/jiami.dog/wp-content/uploads/2021/12/2021-12-28_12-46-27-1.jpg?resize=1577%2C246&ssl=1)
